...
public class CommonLogPermissions : ILogPermissions
{
    ...
    public Collections.Generic.Dictionaries.Dictionary<LogTypes, InheritablePolicy> LogTypesPermissions
    {
        get;
    } =
        new(InheritablePolicy.Inherit);
    ...
    public static CommonLogPermissions LoadFromConfig()
    {
        try
        {
            ...
            rval.LogEnabled =
                Common.Convert.Utils.TryParseTo(xml.GetNodeProperty("/CommonLogPermissions/LogEnabled", x => x.InnerText, "false"), false);
            rval.DefaultPolicy = Common.Convert.Utils.TryParseTo(xml.GetNodeProperty("/CommonLogPermissions/DefaultPolicy", x => x.InnerText, "Deny"), Policy.Deny);
            rval.LogTypesPermissions.CopyFrom(xml.TrySelectNodes("/CommonLogPermissions/LogTypesPermissions/Item")
                                                 .Select(n =>
                                                  {
                                                  ...
                                                  }).Where(i => i.Result).Select(i => new KeyValuePair<LogTypes, InheritablePolicy>(i.LogTypes, i.Policy))
                                                 .Distinct(i => i.Key.ToString()).ToDictionary(i => i.Key, i => i.Value));
            rval.CategoriesPermissions.CopyFrom(xml.TrySelectNodes("/CommonLogPermissions/CategoriesPermissions/Item")
                                                   .Select(n =>
                                                    {
                                                    ...
                                                    }).Where(i => i.Result).Select(i => new KeyValuePair<string, InheritablePolicy>(i.Name, i.Policy))
                                                   .Distinct(i => i.Key.ToLowerInvariant()).ToDictionary(i => i.Key, i => i.Value));
            return rval;
        }
        catch (Exception exc)
        {
