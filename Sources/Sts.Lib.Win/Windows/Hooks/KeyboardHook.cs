...
public class KeyboardHook : HookHelper
{
    ...
    public event EventHandler<ReadonlyDataArgs<(int KeyCode, KeyboardMessageTypes MessageType)>> KeyboardHookEvent;

    protected override void OnHook(UIntPtr wParam, IntPtr lParam)
    {
        KeyboardHookEvent?.Invoke(this,
                                  new ReadonlyDataArgs<(int KeyCode, KeyboardMessageTypes MessageType)>((
                                                                                                            Marshal.PtrToStructure<KeyboardLowLevelHookStruct>(lParam).vkCode, (KeyboardMessageTypes)wParam)));
    }
