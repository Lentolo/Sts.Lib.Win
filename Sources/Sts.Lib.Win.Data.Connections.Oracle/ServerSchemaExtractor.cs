...
namespace StsLibWin.Data.Connections.Oracle
{
    public class ServerSchemaExtractor : IServerSchemaExtractor
    {
        ...
        public ReadOnlyCollection<ColumnItem> GetTableColumns(Table table)
        {
            ...
            return _tableColumns.Value.Where(i => string.Compare(i.TABLE_NAME, table.Name, StringComparison.OrdinalIgnoreCase) == 0).Select(r => new ColumnItem(r.COLUMN_NAME, types[r.COLUMN_NAME], r.DATA_TYPE, r.NULLABLE == "Y", (long)r.DATA_LENGTH, (long?)r.DATA_PRECISION ?? 0, (long?)r.DATA_SCALE ?? 0, r.DATA_DEFAULT, false)).ToList().AsReadOnly();
        }
